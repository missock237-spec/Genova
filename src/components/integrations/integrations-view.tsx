...
const CATEGORY_META: Record<string, { label: string; icon: React.ElementType; color: string }> = {
  ...
  Productivity: { label: 'Productivité', icon: Calendar,      color: 'text-violet-400' },
  ...
  Shipping:     { label: 'Livraison',     icon: Send,          color: 'text-cyan-300' },
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const [catalogRes, connectedRes] = await Promise.all([
        ...
        apiFetch<ConnectedPlugin[]>('/api/plugins/connected').catch(() => []),
      ]);
